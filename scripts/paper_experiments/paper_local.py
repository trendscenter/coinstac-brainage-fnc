...
sys.path.append('/computation/scripts/core')
import common_functions as cf
import preprocessor_utils as preut
import svr_utils as svrut
import paper_utils as paput
...
def local_0(args):
    ...
    owner = state_list["owner"] if "owner" in state_list else owner_node

    input_list["split_type"] = curr_split_type

    [X_train, y_train] = paput.form_XYMatrices_from_csv(input_dir=os.path.join(input_dir, input_list["split_type"]),
                                                        input_file=state_list["clientId"]+"_fnc_age_train.csv")
    [X_test, y_test] = paput.form_XYMatrices_from_csv(input_dir=os.path.join(input_dir, input_list["split_type"]),
                                                      input_file=state_list["clientId"]+"_fnc_age_test.csv")
...
if __name__ == "__main__":
    owner_node="local0"
    curr_split_type = "age_range_stratified"  # All split types: "random", "age_stratified", "age_range_stratified"
